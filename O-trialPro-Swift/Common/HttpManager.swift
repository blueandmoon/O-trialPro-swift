...
//    func Put(path: String, params: Dictionary<String, String>, success: @escaping ((_ result: String?, _ mes: String?) -> ()), failure: @escaping ((_ error: Error) -> ()))
    ...
    func downLoad(_ path: String, params: Dictionary<String, String?>?, success: @escaping ((_ result: Data) -> ()), failure: @escaping ((_ error: Error) -> ())) {
        var path = HttpHelper().checkPathHeader(path)
        let list  = NSMutableArray()
        if params != nil {
            //拆分字典,subDic是其中一项，将key与value变成字符串
...
//        let session = URLSession(configuration: config)
        ...
        Alamofire.request(request).responseData { (dataResp) in
            if dataResp.error != nil {
                failure(dataResp.error!)
                return
            }
            success(dataResp.data!)
        }
